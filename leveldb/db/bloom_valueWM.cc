...
void bloom_valueWM::insert(const std::string& key) {
...
void bloom_valueWM::saveToFile(const std::string& filename) {
...
bloom_valueWM bloom_valueWM::loadFromFile(const std::string& filename) {
...
void bloom_valueWM::createFile(const std::string& filename) {
  std::ofstream file(filename, std::ios::binary);
  if (!file) {
    std::cerr << "Error opening file: " << filename << std::endl;
    return;
...
bool bloom_valueWM::exists(const std::string& key) const {
